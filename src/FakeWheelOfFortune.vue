...
export interface Contestant {
	...
	score: number
	total: number
	inventory: Array<InventoryItems | string>
}
...
const boardAnimationTime = 10 * 1000
...
export default class FakeWheelOfFortune extends Vue {
	...
	private privateContestants: Contestant[] = [
		{ name: 'Ringo', score: 0, total: 0, inventory: [] },
		{ name: 'Jamie', score: 0, total: 0, inventory: [] },
		{ name: 'Remi', score: 0, total: 0, inventory: [] }
	]

	public privateCurrent: number = 0
	public guessValue: number = 0
	public ignoreNextRemoteSpin = false

	public get contestants() {
		return this.privateContestants || []
	}

	public set contestants(contestants: Contestant[]) {
		this.privateContestants = contestants
	}
	...
	public set current(current: any) {
		this.privateCurrent = parseInt(current)
		xhr.post('http://localhost:6226/wheel-of-fortune/current?data=' + parseInt(current))
	}
	...
	private pollServer() {
		setTimeout(async () => {
			if (this.view === 'wheel') {
				try {
					// This service supports long polling
					const data = await xhr.get('http://localhost:6226/wheel-of-fortune/spin')
					if (data.spin && !this.ignoreNextRemoteSpin) {
						this.spin(data.spin)
					}
					this.ignoreNextRemoteSpin = false
				} catch {}
			} else {
				this.ignoreNextRemoteSpin = true
			}
			this.pollServer()
		}, 50)
	}
	...
	public letter(search: string, delay = foundDelay, vowel = false) {
		...
		found.forEach(([i, j], n) => {
			...
			setTimeout(() => {
				...
				if (!vowel) {
					this.currentContestant.score = parseInt('' + this.currentContestant.score, 10) || 0
					this.currentContestant.score += this.multiplier * parseInt(this.guessValue.toString(), 10)
				}
			}, delay * n)
		})
	...
	public buyVowel(vowel: string) {
		if (this.currentContestant.score >= vowelCost) {
			...
			this.letter(vowel, foundDelay, true)
		}
	}
	...
	public switchContestant(n: number) {
		this.current = (this.current + this.contestants.length * 100 + n) % this.contestants.length
	}
	...
	public spin(speed?: number) {
		(this.$refs.wheel as WOFWheel).spin(speed)
	}
